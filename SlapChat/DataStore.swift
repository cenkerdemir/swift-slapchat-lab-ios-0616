...
class DataStore {
    ...
    func generateTestData() {
        let messageDescription = NSEntityDescription.entityForName("Message", inManagedObjectContext: self.managedObjectContext)
        
        if let messageDescription = messageDescription {
            let newMessage = Message(entity: messageDescription, insertIntoManagedObjectContext: self.managedObjectContext)
            newMessage.content = "first message"
            newMessage.createdAt = NSDate()
            self.managedObjectContext.insertObject(newMessage)
            
            let secondMessage = Message(entity: messageDescription, insertIntoManagedObjectContext: self.managedObjectContext)
            secondMessage.content = "another message"
            secondMessage.createdAt = NSDate()
            self.managedObjectContext.insertObject(secondMessage)
        }
        self.saveContext()
        self.fetchData()
    }
